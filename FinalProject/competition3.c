...
void transportingObject()
{
    ...
    /* the robot could have already been at drop zone because it 
    found a light so atDropZone would have already been true */
    if(!atDropZone)
    {
        atDropZone = TRUE;
        alloff();

        // at the intersection, move forward a little bit 
        fd(LEFT_MOTOR);
        fd(RIGHT_MOTOR);
        timeOut = seconds() + 1.0;
        while(timeOut > seconds());
